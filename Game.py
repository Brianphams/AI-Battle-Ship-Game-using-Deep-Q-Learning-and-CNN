...
class HuntingStrategy:
    ...
    def update(self, move, is_hit, is_ship_sunk):
        ...
        if is_hit:
            if is_ship_sunk:
                # Reset nếu tàu đã chìm
                self.reset()
            else:
                # Thêm hit mới
                self.hits.append(move)
                if len(self.hits) >= 2:
                    # Xác định hướng của tàu
...
class Game:
    ...
    def takeAMove(self, next_move=None):
        ...
        if next_move is None:
            # Thử hunting strategy trước
            hunt_moves = self.board.hunting_strategy.get_hunt_targets(available_moves)
            if hunt_moves:
                # Nếu có hunting moves, chọn một trong số chúng
                next_move = random.choice(hunt_moves)
            else:
                # Nếu không có hunting moves, sử dụng model
