...
class BattleshipGUI:
    def __init__(self, model_file=None):
        ...
        # Initialize two separate games for AI and human
        self.ai_game = Game(BOARD_WIDTH, BOARD_HEIGHT, SHIPS, network=self.network)  # AI's ships
        self.human_game = Game(BOARD_WIDTH, BOARD_HEIGHT, SHIPS, network=self.network)  # Human's ships
        # Game state
        self.game_over = False
        self.winner = None
        # Separate move histories for each grid
        self.ai_grid_moves = set()  # Moves made on AI's grid
        self.human_grid_moves = set()  # Moves made on human's grid
    ...
    def get_cell_from_mouse(self, mouse_pos):
        x, y = mouse_pos
        grid_x_start = self.MARGIN  # Start        if grid_x_start <= x <= grid_x_start + self.GRID_SIZE * self.CELL_SIZE and self.MARGIN <= y <= self.MARGIN + self.GRID_SIZE * self.CELL_SIZE:
        col = int((x - grid_x_start) // self.CELL_SIZE)    
        row = int((y - self.MARGIN) // self.CELL_SIZE)
        if 0 <= row < self.GRID_SIZE and 0 <= col < self.GRID_SIZE:
            print(f"Mouse click detected at grid position: ({row}, {col})")  # Debug
            return row, col
        print(f"Mouse click out of grid bounds: {mouse_pos}")  # Debug
        return None
...
if __name__ == "__main__":
    game = BattleshipGUI(r'C:\Users\baoph\Downloads\BattleShip\BattleShip\mymodel.keras')
    game.play()